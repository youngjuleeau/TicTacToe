...
//Objective C framework so we need to conform to NSObject, but also want to add published properties we want to observe changes on so we can update our uI, so also conform to ObserbableObject protocol.
